...
async function run() {
  ...
  fs.writeFileSync('scripts/figma_business_plan_57158_10712.json', JSON.stringify(data, null, 2));
  console.log('Saved JSON to scripts/figma_business_plan_57158_10712.json');
  ...
  if (imgRes.ok) {
    ...
    if (downloadUrl) {
      ...
      fs.writeFileSync('scripts/figma_business_plan_57158_10712.png', buffer);
      
      const artifactDir = 'C:\\Users\\Siraj\\.gemini\\antigravity-ide\\brain\\f6f1908a-1819-4fb8-8e76-982b5b1f16b1';
      fs.writeFileSync(path.join(artifactDir, 'figma_business_plan_57158_10712.png'), buffer);
      console.log('Saved PNG to scripts/figma_business_plan_57158_10712.png and artifact folder');
    }
  }
  ...
  dump(root);
  fs.writeFileSync('scripts/figma_business_plan_dump.txt', lines.join('\n'));
  console.log('Saved dump to scripts/figma_business_plan_dump.txt. Total elements:', lines.length);
}
