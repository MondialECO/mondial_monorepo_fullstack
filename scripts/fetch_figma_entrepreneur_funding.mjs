...
async function fetchNode() {
  ...
  fs.writeFileSync('scripts/figma_entrepreneur_funding_node.json', JSON.stringify(data, null, 2));
  console.log('Saved node data to scripts/figma_entrepreneur_funding_node.json');
  ...
  if (node.children) {
    node.children.forEach((c, i) => {
      console.log(`Child ${i}: "${c.name}" (id: ${c.id}, type: ${c.type}, bounds: w:${c.absoluteBoundingBox?.width}, h:${c.absoluteBoundingBox?.height})`);
    });
  }
}
