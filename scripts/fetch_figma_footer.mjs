...
async function fetchFigmaNode() {
  ...
  fs.writeFileSync('scripts/figma_node_data.json', JSON.stringify(nodeData, null, 2));
  console.log('✓ Node JSON saved to scripts/figma_node_data.json');
  ...
  if (imgRes.ok) {
    ...
    if (renderedImgUrl) {
      ...
      fs.writeFileSync('public/figma_footer_preview.png', Buffer.from(arrayBuffer));
      console.log('✓ Saved preview image to public/figma_footer_preview.png');
    }
  }
}
