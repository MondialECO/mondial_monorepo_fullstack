...
async function main() {
  ...
  for (const vp of [{ width: 1440, height: 1100, name: '1440px' }, { width: 1920, height: 1100, name: '1920px' }]) {
    ...
    if (page.url().includes('/login')) {
      console.log(`[Viewport ${vp.name}] Logging in as demo creator...`);
    ...
    const fullPagePath = path.join(artifactDir, `market_study_full_${vp.name}.png`);
  ...
  await browser.close();
  console.log('Full page captures and PDF export preview saved!');
}
