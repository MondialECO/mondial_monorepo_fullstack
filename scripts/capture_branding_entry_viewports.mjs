...
async function main() {
  const testEmail = `branding.test.${Date.now()}@mondial-test.eco`;
  ...
  for (const vp of viewports) {
    // Light mode
    {
      ...
      await page.goto(`${APP_BASE}/dashboard/creator/phase-2/branding`, {
        waitUntil: 'domcontentloaded',
      });

      await page.waitForSelector('text=Brand Visual Identity Studio', { timeout: 15000 });
      await page.waitForTimeout(1000);

      const screenshotPath = path.join(OUTPUT_DIR, `branding_${vp.name}_light.png`);
    ...
    // Dark mode
    {
      ...
      const screenshotPath = path.join(OUTPUT_DIR, `branding_${vp.name}_dark.png`);
      await page.screenshot({ path: screenshotPath, fullPage: true });
      console.log(`Saved ${screenshotPath}`);
      await context.close();
    }
