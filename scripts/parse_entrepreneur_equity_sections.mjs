...
const data = JSON.parse(fs.readFileSync('scripts/figma_entrepreneur_equity_node.json', 'utf8'));
const root = data.nodes['56877:99210'].document;

const outDir = 'scripts/entrepreneur_equity_sections';
...
function dumpNode(node, depth = 0) {
  ...
  if (node.type === 'TEXT') {
    ...
    text += `${indent}[TEXT] "${name}" (id: ${id}, w: ${w}, h: ${h})${extra} -> ${chars}${font}${extra}\n`;
  } else {
    text += `${indent}[${type}] "${name}" (id: ${id}, w: ${w}, h: ${h})${extra}\n`;
  }
