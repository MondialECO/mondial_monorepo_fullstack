...
**File download security debt (2026-08-03, Path A shipped in commit `93c7e82`):** workroom file download uses `app.UseStaticFiles()` at `backend/Program.cs:660` to serve `wwwroot/uploads/documents/{guid}.ext` files directly. The download URL is `{apiBase}{file.storagePath}`. NO authentication or authorization check exists — anyone in possession of a `storagePath` value (which is exposed via `WorkroomFileResponse` API responses) can download any workroom file, including files marked `providerPrivate: true`. This is a known security debt from a deliberate Path A vs Path B tradeoff made 2026-08-03. Must be replaced with an auth-gated endpoint `GET /api/workroom/files/{id}` before any real-world usage or production deployment involving actual client-provider engagements. The proper endpoint must: (a) require authentication, (b) verify the caller is a participant in the file's engagement (client OR provider), (c) enforce `providerPrivate` — private files only reachable by the provider. Backend static file serving of `wwwroot/uploads/documents/` should also be disabled when the auth-gated endpoint ships, to prevent bypassing the check via direct URL.

Two clarifications to the entry above, recorded at the same time. There is no `WorkroomFileResponse` DTO — `WorkroomDetailResponse.Files` is `List<WorkroomFile>`, the raw BSON model, which is *why* `StoragePath` reaches the client at all. And this is not a new exposure: "Workroom internal storage-path exposure" was already carried as Medium technical debt in §8 and the §17 rollup. What changed on 2026-08-03 is that the UI now links to those paths, so the exposure moved from theoretical to reachable by any user of the product.
