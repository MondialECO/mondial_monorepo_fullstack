...
**Green text — `--success-strong`, not `--success-text` *(added 2026-08-04)*.** Two green tokens exist and they are not interchangeable. `--success-text` (`#00A854`) measured against `--success-light` (`#E5F7ED`) is **2.80:1**, which fails AA for body copy *and* the 3:1 threshold for large text and UI. `--success-strong` (`#157A55`) is **4.78:1** and passes. Green **text** on a light surface must use `--success-strong`; `--success-text` is for accents on white and for fills. A "consistency" sweep unifying the two is an accessibility regression, not a cleanup, and is pinned by test on the Earnings surface. The Earnings surface (`EarningsWorkspace` + the three earnings panels) is **fully token-driven with zero hex literals**; `AnalyticsWorkspace` still carries deliberate literals with no exact token equivalent — see the PENDING DESIGN-TOKEN DECISION note in `globals.css`.
...
**Category overlap — these do NOT sum *(documented 2026-08-04)*.** Traced through `GetProviderFinancialSummaryAsync`: `ProtectedEscrow` admits 13 of the 15 `WorkroomMilestoneStatus` members, and `WorkInProgress`'s three (`Funded`, `Active`, `SubmissionDraft`) are all inside that set — so **Work in Progress is a strict subset of Protected Escrow**, not a partial overlap. Funded `InReview` milestones are inside it too, and `Pending` straddles it (`PaymentProcessing` inside, `Approved` excluded). Adding the categories therefore overstates a provider's money. Any UI presenting them together must present Protected Escrow as a **total that the stage figures break down**, never as a peer figure beside them.

**UI presentation *(as of 2026-08-04)*.** The Earnings Overview tab displays six figures — Gross earnings, Fixed platform commission, Net earnings, Pending release, On hold, Withdrawn — plus a hero Available-balance card carrying the payout CTA. `WorkInProgress`, `InReview` and `ProtectedEscrow` are **computed and returned by the API but not currently displayed**; the section that showed them was removed. Consequence to preserve: milestones in **revision or dispute** were only ever visible inside the Protected Escrow total, so those amounts are currently not represented anywhere in the SP UI. Restoring them is a frontend-only change — no field was dropped.
...
**Balance calculations *(corrected against code 2026-08-04)*:**
- `Available = max(0, Released Earnings − Completed Payouts − Active Payout Requests + Completed Adjustments)` — the clamp is real; a negative available balance is never returned.
- `Pending = Σ milestone amounts where status ∈ {Approved, PaymentProcessing}` — a status-scoped sum, **not** the subtraction previously documented here.
- `Protected Escrow = Σ milestone amounts where escrow = Funded AND status ∉ {Approved, Paid}` — the exclusion is `Paid`, **not** `Refunded` as previously documented.
- `Work in Progress = Σ where escrow = Funded AND status ∈ {Funded, Active, SubmissionDraft}`
- `In Review = Σ where status ∈ {Submitted, ClientReviewing, Resubmitted}` — **not** escrow-filtered, so a submitted milestone whose escrow moved to `OnHold` counts under On Hold and not here.
- `On Hold = Σ where escrow = OnHold`; `Withdrawn = Σ completed payouts`.
...
- **`notTracked` is an honest data-source state, not a bug or fabricated zero:** unavailable metrics carry `State = "notTracked"`, null numeric/comparison/change values, and a reason in the API; the UI labels them “Not tracked yet.”
  ...
  - **Date-filtered Service counters — RESOLVED 2026-08-04 (`4f5fc74`).** Impressions, service views/clicks, click-through rate and conversion rate are now sourced per listing from `AnalyticsDailyBuckets`, which Phase A/B/C has been writing since 2026-07-31. Module 5 had continued to hardcode them `notTracked` against a reason that stopped being true once that tracking shipped. Three product decisions are recorded in §9.4.11. **`Enquiries` and `EnquiryConversion` remain `notTracked`** — no enquiry entity exists, which is a different and still-true reason — and **`Custom/Unattributed` rows stay `notTracked` for all four traffic fields**, because a row with no `ServiceId` has no `ListingId` to aggregate against.
...
- **Dashboard envelope:** `Period, Currency, AvailableCurrencies, HistoryStartedAt, HasMinimumHistory, IncludesRecordsWithoutTestProvenance, DataLimitation, Overview, Services, Proposals, Profile, Revenue, Clients, Observations, UnavailableObservationRuleIds, EmptyStates, Trend[], TrendGranularity`.
- **Trend series *(added 2026-08-04)*:** `AnalyticsTrendPoint = { PeriodStart, Label, NetEarnings, AverageRating }`. Bucket width is the server's decision (`AnalyticsTrendBuckets.GranularityFor` → `day`/`week`/`month` from the span), so consumers report the granularity rather than assuming weeks. `NetEarnings` is a real 0 for a period with no releases; `AverageRating` is **null, never 0**, when no review was submitted — the average of nothing is not zero. The series applies the same refunded-milestone exclusion as `Revenue.Net`, and their sums are pinned equal by test.
...
- **As-built Revenue additions *(2026-08-04)*:** `ClientSource = { EcosystemMatch, MarketplaceSearch, EcosystemNet, MarketplaceNet, UnattributedNet }`. Released revenue split by how the client arrived, collapsing `ProposalSource` into two channels — **Ecosystem Match** (`StandardProposal`, `DirectInvitationProposal`, `CustomOffer`) and **Marketplace Search** (`PublishedPackagePurchase`, `PackageAddOn`, `ChangeRequest`). Weighted by **net revenue**. Attribution walks transaction → engagement → proposal, the same chain `ServiceIdentity` uses, over the already refund-filtered release set. Revenue that cannot be traced to a proposal is **excluded from the percentages** and surfaced as `UnattributedNet` rather than folded into a channel. Zero attributable revenue is `notEnoughActivity`, never a 50/50 or 0%/0% split.
- **As-built Clients additions *(2026-08-04)*:** `Origination = { EcosystemMatch, MarketplaceSearch, EcosystemClients, MarketplaceClients, UnattributedClients }` — the same two channels counted by **distinct client**, deliberately unlike Revenue's money weighting. Each client is attributed to the channel behind their **earliest engagement in the period**, so the two counts partition the client set and a returning client is counted once. `RatingDistribution[] = { Rating, Count }` — always five buckets (1–5) including empty ones, with `TotalReviews` separating "nobody rated 5" from "nobody rated"; ratings outside 1–5 are ignored, never clamped. `TopIndustries[] = { Industry, Projects }` — resolved engagement → proposal → originating `ClientBrief.Industries`, capped at five. A brief naming several industries **counts once in each**, so these counts do **not** sum to the project total; work with no brief behind it groups under `Custom/Unattributed` (§9.1's existing label). `MostActiveClients[]` gains `AverageRating`, **null and never 0** when the client left no verified review — an unrated client has not rated you badly. Client IDs stay masked by `MaskClient`.
- **Shared classifier:** both splits call one `IsEcosystemMatch`, a switch with a throwing default, so an added `ProposalSource` member must be classified deliberately rather than defaulting into Marketplace — and the two tabs cannot drift into classifying the same client differently. Pinned by a test walking every enum member through both.
...
**Revenue analytics:** gross, commission, and net derive only from non-refunded completed release rows; current balances reuse Module 4's amount categories; breakdowns cover service, category, masked client ID, and month. All values are currency-scoped. The **client-source split** (§9.0.1) is weighted by revenue rather than deal count, deliberately: it sits on the Earnings tab beneath Total Earnings where every other figure is money, and the two bases disagree exactly where it matters — ten small package purchases against two large matched projects is a majority of *deals* to Marketplace and a majority of *income* to Ecosystem.

**Client analytics:** period clients/new/returning and period repeat revenue sit alongside lifetime-as-of-period repeat rate, average projects/client, and lifetime value. Verified reviews supply rating averages and the five-bucket rating histogram. Client identifiers are masked in the most-active list, which also carries a per-client average rating. Repeat/on-time formulas use the shared calculator, not an analytics copy. **Client origination** counts heads, not money — this tab answers where relationships come from, while Revenue's split answers where income comes from; both use the same channel classifier. **Top industries** attributes completed work through the originating brief, and its counts overlap by design (§9.0.1), so any UI must scale those bars against the largest row rather than a total and must not present them as shares of a whole.
...
#### 9.4.11 Module 5 consumes the buckets *(2026-08-04, `4f5fc74`)*

Phases A–D wired real tracking on 2026-07-31, but only the **Phase A–D dashboard** at `/dashboard/serviceprovider/services` read it. The **Analytics & Growth** service view is a separate surface built by `AnalyticsService.BuildServiceAnalytics`, and it went on hardcoding `Impressions`, `ServiceViews`, `ClickThroughRate` and `ConversionRate` as `notTracked` against a reason that had stopped being true. Both surfaces now read the same collection.

**Three product decisions, encoded in `ServiceTraffic` rather than at the call site:**

1. **`ServiceViews` aliases Clicks, not Impressions.** `AnalyticsDailyBucket` has no views counter — an impression *is* a detail-page load — and the frontend column is labelled "Traffic" showing clicks. Aliasing to impressions would have double-reported the same number under two names.
2. **`ConversionRate` here is Orders ÷ Impressions.** Deliberately a *different* metric from the Phase A–D dashboard's `ConversionRate`, which is Inquiries ÷ Impressions and is unchanged. The service row sits beside an `Orders` column and a separate `EnquiryConversion`, so orders is the reading that makes the row coherent. **The same field name means different things on the two surfaces** — check which one you are looking at before comparing them.
3. **Windows round outward to whole UTC days, disclosed when it happens.** Buckets are one row per listing per day, so a window that starts or ends mid-day cannot be answered exactly. Rounding outward never hides traffic; the widening is appended to the metric's `reason`, and only when it actually occurred.

**Correction to the scoping assumption:** it was expected that only Custom Range needed rounding. **It does not hold.** `AnalyticsPeriodResolver` sets `to = now` for `ThisMonth`, `Last7Days`, `Last30Days`, `Last90Days` and `ThisYear`, and the `Last*Days` ranges derive `from` from `now` too, so both ends carry a wall-clock time. **`PreviousYear` is the only range already day-aligned.** Rounding is therefore applied uniformly and disclosure is driven by measured alignment, never by range name.

**Boundary convention — half-open `>= From && < To`, matching Module 5.** The Phase A–D summary endpoint uses an *inclusive* end (`<= end`) instead. That mismatch is the kind of thing that let this gap persist, so rather than copying Phase A–D's query, the bucket read adapts to Module 5's own convention — the one its ~15 other window predicates already use. **Rule:** one boundary convention per module; adapt the query to the module, not the module to the query.

**Still `notTracked`, correctly:** `Enquiries` and `EnquiryConversion` (no enquiry entity exists), `CancellationRate` (no durable cancellation history), and **all four traffic fields on `Custom/Unattributed` rows** — a row with no `ServiceId` has no `ListingId` to aggregate against, so it reports untracked rather than a misleading zero.

A listing with no bucket rows reports a real **0** impressions, which is different: it was surfaced and nobody looked. Its rates report `notEnoughActivity` rather than 0%, since there is no denominator.

**Overview tab:** `ProviderDashboardServiceViewsResponse` carries only `Impressions` and `Clicks` — not the three-metric service-row shape — and is now provider-wide over the same rolling 30-day window as its sibling fields.

**Known UI gap, not fixed here:** the ServicesView table has one traffic column ("Traffic" = clicks). `Impressions`, `ClickThroughRate` and `ConversionRate` are now returned by the API but have **no column to render in**, so the fix is currently invisible except for that column and the `TrackingGaps` panel shrinking. Adding columns is a layout decision, deliberately left out of a data fix.
...
**Rule:** any new "is a dispute active" check gates on `DisputeOutcome == Open`, never on `DisputeOpenedAt != null`.

**Dispute resolution — client-favoured milestone semantic (2026-08-02, commit `3420a4f`).** On client-favoured resolution the milestone transitions to `Paid` — the payment-settled terminal state — with `RefundedAt` set. Funds return to the client via the existing `FinancialTransactions` refund entry; the milestone does **not** transition to `Cancelled`. This preserves the "all milestones `Paid`" precondition in `CompleteEngagementAsync`, so an engagement completes normally after a client-favoured dispute.

`Paid` therefore means **payment settled in either direction**. `Paid && RefundedAt == null` is the standard released-to-provider case; `Paid && RefundedAt != null` is the refund-completed case. `RefundedAt` is the only discriminator, so anything summing provider revenue must exclude it — as of `3420a4f` every such figure already does, by gating on `EscrowStatus` or on `PaymentReleased` transactions rather than on milestone status. UI must render **"Refunded"** as the primary label whenever `RefundedAt` is set; helpers `isRefundedMilestone` / `milestoneStatusLabel` / `milestoneChipClass` (`src/lib/workroom-status.ts`) and `milestoneBadge` (`_shared.tsx`) exist so this is not re-derived per component.

A new enum state was deliberately **not** added: every guard that tests for `Paid` would need a matching edge, which is precisely how `Cancelled` became unreachable. `Disputed → Paid` was added to the milestone transition map, which previously permitted only `ClientReviewing` and `Cancelled`.

Prior to `3420a4f`, client-favoured resolutions set the milestone to `Cancelled`, creating an engagement that could never complete, cancel, or archive while still consuming provider capacity — audit finding BUG-2, now fixed. `WorkroomMilestoneStatus.Cancelled` consequently has **no writer**, and `EngagementStatus.Cancelled`/`Archived` still have none: there remains no cancellation or archival path anywhere in the system.
...
**File download — Path B shipped 2026-08-03, commit `3b11c98` (frontend `eb08d8e`).** Workroom files are served by the auth-gated `GET /api/workroom/files/{id}/download` on the `[Authorize]`d `WorkroomController`. It streams via `PhysicalFile` with range processing, sets `Content-Type` from the stored `ContentType` and `Content-Disposition` from `OriginalName`. Authorization lives in `WorkroomFileAccess.Evaluate`, a pure function covered by 14 unit tests: a non-participant and a nonexistent file both return `Denied` and map to **404**, so the endpoint cannot be used to probe for file ids; `ProviderPrivate` files are denied to the client even though they participate in the engagement; privacy is checked before readiness so status cannot leak through the difference; only `Ready` files are served, enforcing server-side the same gate `isFileDownloadable()` applies in the UI. The resolved path is re-checked against the documents root so a traversal sequence in a stored `StoragePath` cannot escape it.

Direct static access to `/uploads/documents` is blocked by middleware placed before `app.UseStaticFiles()`, which returns 404 — the same response a nonexistent path gives, so the folder is indistinguishable from absent rather than merely forbidden. Every other upload folder (media, profile, `service-provider/*`, branding, identity) is untouched and still served statically. The frontend calls the endpoint through the shared axios instance so the bearer token and refresh interceptor apply; `workroomFileDownloadUrl()` is deleted.

**Residual, not yet done:** the documents folder is still physically inside `wwwroot`, so the protection is a deny rule ahead of the static handler rather than an absence of any route to the bytes. Moving it outside the web root would make middleware order irrelevant; that requires a `SaveFile` change plus a migration of existing files and is deliberately deferred.
...
Cross-cutting visual and semantic-token polish across all four client-facing marketplace surfaces plus the SP-side workroom. Independent of the M-phase numbering — no functional lifecycle change, no backend change (except the M2-era `RequirementsTemplate` DTO exposure that shipped earlier). Fiverr-inspired dense marketplace vocabulary, adapted to Mondial's canon-locked light theme + Inter/DM Sans + `#3C61DD` primary via `bg-primary` Tailwind token (never hex literals in components).

**Phase R1 — Marketplace grid.** Sticky left filter sidebar (240px desktop, mobile Sheet drawer). Filter groups render as vertical checkbox/button lists (not dropdowns) but stay single-select internally due to `MarketplaceListingsQuery` shape (backend constraint). Sort dropdown next to results header, not in sidebar (Fiverr convention). Cards use 3-column grid with sidebar, aspect-video cover with subtle `hover:scale-105`, Inter 600 title, single-line price + delivery bottom row, provider tooltip on hover (desktop only). No fake data — rating markup gated on backend-provided `rating != null`. Commits: `b99bdeb`, `09ea826`, `e3da4d9`, `bfabd8e`.

**Phase R2 — Listing detail.** Two-column layout (`grid-cols-[1fr_360px]`) with header block extracted above columns, sticky right-column package selector card. Compare-packages table introduced (feature union across all tiers, ✓/✗ per package, Choose buttons per column). Provider info split into compact header row (orientation) and expanded "About the provider" card at bottom of left column (handshake). Package selector card restyled: shadcn `<Tabs>` for tier switching, included/excluded features consolidated into one list with mixed check/X icons, Total + CTA block ("Continue (€X)" label). Compact provider strip removed from top of sticky selector (deliberate deduplication with header + about card). Commits: `36d4f43`, `7b93086`, `85fffd2`, `7d4f59e`, `e202215`.

**Phase R2.5 — Semantic token cleanup + `--rating` infrastructure.** Verified badges + included-feature check icons + order-flow success confirmations migrated to green success pair (`Badge variant="success"` + `text-success-text`, values `--success-light: #E5F7ED` + `--success-text: #00A854`). Recommended pill on package tabs stays primary blue. Excluded-feature X marks stay `text-muted-foreground`. Extended to grid card + order flow success screens for cross-page consistency. Rule locked: **same feature list from same DTO field must render same color everywhere** (this rule overrides the "brand affiliation vs success" distinction when they conflict). `--rating: #F59E0B` token defined in `globals.css` and exposed via `@theme inline` for future star migration; no star usage migrated yet — `MarketplaceCard.tsx` and `ReviewPanel.tsx` still on `text-amber-400 fill-amber-400`. Orphan `--success-text` triplet duplicates removed from `:root` and `.dark` blocks. Commits: `c825285`, `976d858`, `79d97fb`.

**Phase R3 — Order flow (3-step wizard).** Two-column layout (`grid-cols-[1fr_320px]`) with sticky right-column order summary card (mirrors R2 pattern). New wizard progress stepper: 3 circles with connecting lines (solid primary for completed segment, dashed muted for upcoming), completed circle shows check icon, current filled, future outlined. Summary card: cover thumbnail + provider + service title + tier badge + line items + Total + commission preview info box ("Provider receives €X after 12% Mondial platform fee"). Step cards get consistent header/body/footer wrapper (`rounded-xl border bg-card p-6-8`). Result panel refactored: auto-accepted, manual-approval, polling-timeout as mutually exclusive states (previously timeout rendered underneath success — now the green success moment cleanly disappears if timeout fires). All purchase-mutation wiring, polling, engagement-resolution redirect, `complianceHold` inversion preserved verbatim. Commits: `a45f2b5`, `6d134ee`, `ccffc41`, `a3710cf`, `6c2fe2f`, plus `9c8bc24` (fix: empty-labelled required requirements field surfacing as an orphan red asterisk beside the subtitle).

**Phase R4 — Engagements list + detail.** List page: wider wrapper (`max-w-6xl`), Fiverr-style status filter tabs (All / Active / In review / Completed / Disputed) with per-tab counts. Active bucket initially over-narrow (spec bug fixed in `e99500a` — every non-terminal engagement state now included, so `ContractPending` and `EscrowPending` engagements no longer disappear except in "All"). Detail page: 2-column with sticky right-column engagement summary card (contract value, escrow status, dates, quick actions relocated from `EngagementActions` via new `stacked` prop). Left column: 4 panels (Contract, Milestones, Files, Review) each in matching card shell. `MilestoneCard` state-branch logic (11 states) preserved verbatim, only container styling + action-button semantic variants (Primary Fund/Approve, Outline Request Revision, Destructive-outline Open Dispute) changed. `workroom-status.ts` tone system migrated: `waiting` → `bg-warning/10 text-warning`, `good` → `bg-success-light text-success-text`. Provider display name omitted from list card and detail header (backend `Engagement` DTO carries only `clientDisplayName`, no `providerDisplayName`) — flagged for future backend addendum. Mobile detail summary renders **below** left content (workroom primary is the work-in-progress, not the price — deliberate inversion vs R3 order flow where summary is above). M3a buyer-scope ownership guard preserved. Commits: `ce74bd5`, `50bc0e6`, `64d8c30`, `375aff0`, `65b3adf`, `ff2969d`, `e99500a`.

**SP Path A — Provider-side workroom fixes (bundled with R sprint).** Three functional/consistency findings from the SP workroom audit:

- **F1 (Resubmitted milestone re-delivery)** — investigated, confirmed non-issue. Backend `SubmitDeliverableAsync` accepts `Active`, `SubmissionDraft`, `RevisionInProgress` only, and `WorkroomStateMachine` disallows `Resubmitted → Resubmitted`. UI mirrors backend precisely; product behavior "provider waits for client after resubmitting" is intentional. No commit.
- **F2 (Approved + PaymentProcessing latent states)** — deferred to canon M4 (state-machine hardening). No writer server-side today, no UI treatment needed.
- **F3 (Disputed engagement UI still offered submission form)** — fixed. Predicate split: `terminalEngagement()` keeps its lifecycle meaning for list categorization (`ProjectList` Active vs Completed tab split — disputed engagements correctly do NOT move to Completed), new `providerActionsLocked()` (terminal OR Disputed) drives `readOnly` in `ProjectDetail`. Mirrors client-side M3 EngagementDetail behavior. Commit: `58df612`.

Warning color consolidation: three warning colors coexisted (`SpStatusBadge` `#965F11`, `SpMutationFeedback` `#D97706`, semantic `--warning: #B45309`). Migrated both SP-primitive inline warnings to `text-warning`/`bg-warning/10`/`border-warning/30` (commit `f81f427`), then swept the remaining 10 files across analytics, earnings, leads, profile, overview, and one missed workroom file (`CoordinationPanel.tsx`) — 14 occurrences total, zero `#D97706` outside `ui/index.tsx` (previous audit's file count was miscounted from an -l grep matching either pattern). Two sites deliberately left as hex: `AnalyticsWorkspace.tsx:483` metric delta direction indicator (semantically a declining metric, not a warning), `ServiceProviderOverview.tsx:183` decorative IconBubble palette (needs whole-palette semantic mapping decision). Commit: `a83b416`. Visual regression noted: `--warning: #B45309` is darker and browner than the migrated hexes, and now theme-aware (dark mode resolves to `#FBBF24`); shipped without a targeted browser regression check of the specific affected surfaces.

Design vocabulary decisions locked platform-wide:

- Fiverr-inspired dense marketplace, always adapted to Mondial canon; never copy Fiverr colors, badges, or upsell patterns.
- Sticky left filter sidebar on grid; sticky right summary on detail / order flow / engagements detail.
- Semantic tokens exclusively for colors (`bg-primary`, `text-success-text`, `text-warning`, `text-muted-foreground`). Never hex literals or Tailwind palette utilities in components.
...
**CSS token bug (out of marketplace scope, documented for whoever fixes onboarding):** `--success-bg` and `--success-border` have no `--color-*` mapping in `@theme inline`, so `UniversalPhase1.tsx` (onboarding banner) using `bg-success-bg`/`border-success-border` emits no styles. Fix is either adding the two mappings, or migrating the component to the properly-wired `success-light`/`success-text` pair now used across marketplace.
...
**2026-08-04 — Module 5 Analytics: client-source, origination, rating histogram, industries, per-client rating.** Commits `0150ced`, `0b88936`, `66b63c6`, `c5d9936`, `c4d3bfc`, `67d3d04`, `4ca561b`. Added `Revenue.ClientSource` (revenue-weighted) and `Clients.Origination` (client-count), both collapsing `ProposalSource` into Ecosystem Match / Marketplace Search through **one shared classifier** so the two tabs cannot disagree; plus `Clients.RatingDistribution[]` + `TotalReviews`, `Clients.TopIndustries[]`, and `AverageRating` on `MostActiveClients[]`. Honest-state discipline preserved throughout: no fabricated 50/50 split, untraceable revenue and clients excluded from ratios rather than assigned, empty rating buckets retained so the histogram keeps its shape, multi-industry briefs counted once per industry with the resulting overlap disclosed rather than hidden, and unrated clients reported as null rather than 0. Frontend: Profile-tab MetricGrid removed as unscoped duplication of the real Profile & Trust page, Verification and Tier merged (they are one causal fact — verification grants Tier 2 and Tier 3/4 have no writer), and the Clients tab's pre-redesign grid, secondary satisfaction tiles and disputes card removed. Aggregation logic extracted to `AnalyticsClientSource` / `AnalyticsClientInsights` following `AnalyticsBucketWindow`'s precedent, because the integration tests require Docker and skip locally — the arithmetic would otherwise have shipped unverified.

**2026-08-04 — Module 5 now reads AnalyticsDailyBuckets; four service metrics stop claiming notTracked.** Commit `4f5fc74`. Phases A–D wired real impression/click tracking on 2026-07-31, but only the Phase A–D dashboard consumed it — the Analytics & Growth service view is a separate builder and went on hardcoding `Impressions`, `ServiceViews`, `ClickThroughRate` and `ConversionRate` as `notTracked` against a reason that had stopped being true. Three decisions: **ServiceViews aliases Clicks** (no views counter exists; the column is labelled Traffic); **ConversionRate here is Orders ÷ Impressions**, deliberately different from the Phase A–D dashboard's Inquiries ÷ Impressions, which is unchanged — the same field name now means different things on the two surfaces; and **windows round outward to whole UTC days with the widening disclosed on the metric**. **Correction:** the assumption that only Custom Range needed rounding was wrong — `AnalyticsPeriodResolver` sets `to = now` for every range except `PreviousYear`, so rounding is applied uniformly and disclosure is driven by measured alignment rather than range name. Boundary convention standardised on Module 5's half-open `>= From && < To` rather than copying Phase A–D's inclusive end, since two conventions in one module is what let the gap survive. `Enquiries`/`EnquiryConversion` and `Custom/Unattributed` rows correctly stay `notTracked`. Frontend `TrackingGaps` no longer lists the four as permanently untracked. **Known gap:** the ServicesView table has no column for Impressions/CTR/ConversionRate, so the fix is largely invisible until columns are added. Suite **786 passed / 85 skipped**, same single pre-existing failure. See §9.4.11.
