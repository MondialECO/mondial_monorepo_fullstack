...
| **Embedded Role Profiles on `ApplicationUser`**<br/>`ApplicationUser.CreatorProfile`<br/>`ApplicationUser.EntrepreneurProfile`<br/>`ApplicationUser.ServiceProviderProfile` | **LEGACY EMBEDDED (RETIRED FOR PROFESSIONAL DATA)** | `ProfessionalProfiles`, `CreatorJourneys`, `Companies` | Universal public/professional profile now lives exclusively in `ProfessionalProfiles`. Embedded professional fields on `ApplicationUser.ServiceProviderProfile` (`Skills`, `Headline`, `Bio`, `Overview`, `Experiences`, `Education`, `Credentials`, etc.) have been retired and unset in DB. SP operational/business fields (`VerificationStatus`, `ProviderTier`, `TrustScore`, `Categories`, `Capacity`, `Financial`, timestamps) stay on `ApplicationUser`. Entrepreneur ventures live in `Companies`. Creator journeys live in `CreatorJourneys`. |
