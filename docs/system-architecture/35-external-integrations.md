...
| **Sumsub KYC** | Automated identity verification, document OCR, and biometric face liveness | `SumsubService.cs` | `Sumsub:AppToken`<br/>`Sumsub:WebhookSecret`<br/>`Sumsub:BaseUrl` | Applicant profile, ID document scans, biometric selfie frames | Verification status (`GREEN`/`RED`), OCR identity fields, webhook notifications |
