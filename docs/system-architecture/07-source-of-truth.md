...
| **Creator HumainX Gate & State** | Profile completeness + Wizard journey | `ProfessionalProfiles` (Data) + `localStorage` (Journey) | `isQuickStartComplete(profile)` & `isQuickStartJourneyComplete(userId)` | **CANONICAL (DUAL-GATE)**. Profile data in MongoDB. Journey confirmation state (`step1Confirmed`, `step2Confirmed`, `step3Confirmed`, `completed`) in client `localStorage`. Dashboard access requires both. Autosave does not confirm steps. |
...
8. **Creator HumainX Dual-Gate Rule**:
   - Creator Dashboard access strictly requires both profile completeness and explicit journey completion: `isQuickStartComplete(profile) && isQuickStartJourneyComplete(userId)`.
   - Profile data is durably stored in MongoDB via `ProfessionalProfiles`.
   - Wizard journey progression (`step1Confirmed`, `step2Confirmed`, `step3Confirmed`, `completed`) is scoped per user ID in browser `localStorage`.
   - Pre-existing profile data prepopulates screens but cannot skip wizard steps.
   - Debounced autosave (400ms) only saves profile data; it must never confirm wizard steps. Only explicit user actions on navigation buttons advance step confirmations.
   - Only the final CTA ('Start my project') on Step 3 completes the journey.
