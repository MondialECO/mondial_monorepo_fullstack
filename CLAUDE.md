...
9. **Creator Brand Kit Studio & Hub rules (Verified Build Clean):**
   - **Routes & Flow:** Branding entry at `/dashboard/creator/phase-2/branding` (single-card presentation max-w-[680px], no icon tile per updated Figma revision, body copy in DM Sans `font-sans`, 6 deliverables checklist, single filled blue "Open Brand Studio" button, quiet "Skip for now" row routing to `/complete`). Studio shell at `/dashboard/creator/phase-2/brand-studio`. Hub page at `/dashboard/creator/phase-2/brand-kit`. Compact summary at `/dashboard/creator/phase-2/complete` (5-role swatch strip, typography pairing, active mark, links to hub/studio). (Legacy `/logo-tool` route retained for backward compatibility).
   - **Viewport Scaling (1440px → 1920px):** Authored at 1440px Figma base; all views stay fluidly responsive from 1440px through 1920px (and down to mobile).
   - **Studio Layout & Navigation:** Persistent View Mode workspace (legacy progress topbar `BrandStudioProgressBar` removed). Streamlined header (`Brand Studio` / `Your Visual Identity — Review and manage your complete brand identity.`). Canvas accumulates persistent result cards with on-demand "Edit" triggers. Modals appear as overlays for creation/editing. Page = persistent overview/control center; Modals = creation and editing tools.
   - **Canonical 7-Stage Flow:** Strategy $\to$ Direction $\to$ Logo Type $\to$ Logo Creation $\to$ Variations $\to$ Colour $\to$ Typography. First-time auto-chains to View Mode; on-demand edit returns directly to View Mode.
   - **Step 1 Strategy Review & Edit Tracking:** `PatchStrategy` updates `.Value`, sets `.EditedAt = DateTime.UtcNow`, and marks `.Provenance = "user_refined"` across all 4 `BrandProvenancedText` fields (`Concept`, `TargetAudience`, `Industry`, `Positioning`). `StrategyReviewModal.tsx` dynamically calculates and displays relative-time `"EDITED X AGO"` badges from real `editedAt` timestamps (unedited fields cleanly display `"From your idea"` with no timestamp). `TonePosition` (Formal $\leftrightarrow$ Casual) and `FirstAppearance` (e.g. `website`, `app_icon`, `invoice`, `social`) are fully implemented and consumed downstream by `LogoTypeChooserModal` (fit score reasoning), `DirectionGenerationService` & `TypographyGenerationService` (prompt tuning), and `BrandKitHubView` (summary facts).
   - **Interface Fonts:** Standardized on Inter (`font-heading`) for titles/headings, DM Sans (`font-sans`) for body copy across all Studio surfaces, JetBrains Mono (`font-mono`) for numerals, tokens, and telemetry badges (Syne Bold is not used for Studio UI).
