...
interface BuyoutHandoverModalProps {
  isOpen: boolean;
  onClose: () => void;
  dealId: string;
  isCreator: boolean;
...
export const BuyoutHandoverModal: React.FC<BuyoutHandoverModalProps> = ({
  isOpen,
  onClose,
  dealId,
  isCreator,
      ...
      <Card className="relative w-full max-w-5xl bg-card border-border rounded-2xl shadow-2xl my-8 overflow-hidden">
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between px-6 py-3 bg-card border-b border-border">
          <span className="text-xs font-semibold text-primary uppercase tracking-wider">
            Full Buyout • Asset Handover &amp; Final Completion
          </span>
          <Button
            variant="ghost"
            size="sm"
...
export default BuyoutHandoverModal;
