  ...
  return (
    <SpPage>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <SummaryMetric icon={ShieldCheck} label="Payment state" value={words(engagement.escrowStatus)} />
      </div>

      <SpTabBar label="Project sections" items={tabs.map(({ label, href, active }) => ({ label, href, active }))} />

      {activeTab === 'overview' && <ProjectOverview data={data} onSelectMilestone={onMilestone} />}
...
function ProjectOverview({ data, onSelectMilestone }: { data: WorkroomDetailType; onSelectMilestone: (id: string) => void }) {
  ...
  const currentIndex = current ? data.milestones.findIndex((item) => item.id === current.id) : -1;
  const progress = Math.max(0, Math.min(100, data.engagement.completionPercentage));
  return (
    <div className="space-y-6">
      <SpCard>
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
            <div><h2 className="font-heading text-xl font-semibold text-[#171717]">Project summary</h2><p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-[#4B5563]">{data.engagement.description || 'No project description.'}</p></div>
