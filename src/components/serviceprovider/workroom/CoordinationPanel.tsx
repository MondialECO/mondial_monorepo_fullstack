...
export function CoordinationPanel({ data, readOnly }: { data: WorkroomDetail; readOnly: boolean }) {
  ...
  const sendInput = async () => {
    ...
    try {
      ...
      setFeedback({ status: 'success', message: 'Request sent to the client. Your deadlines have not moved.' });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <SpCard>
          <SpSectionHeader title="Tasks" description="Notes to keep your own delivery work organised. Once created, a task's status is set by the system — you cannot change it here." action={!readOnly && <Button type="button" size="sm" variant="outline" onClick={() => setTaskOpen(true)}><Plus className="size-4" aria-hidden="true" /> Add task</Button>} />
