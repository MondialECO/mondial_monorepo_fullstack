    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <SpCard>
          ...
          {data.clientInputRequests.length ? <ul className="mt-5 space-y-3">{data.clientInputRequests.map((item) => <li key={item.id} className="rounded-xl border border-[#E5E7EB] p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold text-[#171717]">{words(item.type)}</p><p className="mt-1 text-sm leading-6 text-[#6B7280]">{item.description}</p></div><SpStatusBadge tone={item.status === 'Supplied' ? 'positive' : 'warning'}>{words(item.status)}</SpStatusBadge></div>{item.deliveryImpact && <p className="mt-3 rounded-lg bg-[#FBF2E7] px-3 py-2 text-xs text-[#965F11]">Delivery impact: {item.deliveryImpact}</p>}<p className="mt-3 text-xs text-[#6B7280]">Requested for {formatDate(item.dueDate)}</p></li>)}</ul> : <SpEmptyState icon={UserRoundCheck} title="No client input requested" description="Use this when a file, decision, clarification, or approval is blocking delivery." className="mt-5 min-h-52 border-0 p-0" />}
        </SpCard>
      </div>

      <Dialog open={taskOpen} onOpenChange={setTaskOpen}><DialogContent><DialogHeader><DialogTitle>Create a Workroom task</DialogTitle><DialogDescription>Tasks are coordination records. The current provider API does not support changing their status after creation.</DialogDescription></DialogHeader><SpFormField id="task-title" label="Task title" required><Input value={task.title} onChange={(event) => setTask({ ...task, title: event.target.value })} /></SpFormField><SpFormField id="task-description" label="Description"><Textarea value={task.description} onChange={(event) => setTask({ ...task, description: event.target.value })} rows={3} /></SpFormField><div className="grid gap-4 sm:grid-cols-2"><SpFormField id="task-milestone" label="Milestone"><Select value={task.milestoneId} onChange={(value) => setTask({ ...task, milestoneId: value })}><option value="">Project-wide</option>{data.milestones.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</Select></SpFormField><SpFormField id="task-due" label="Due date"><Input type="datetime-local" min={localDateTime()} value={task.dueDate} onChange={(event) => setTask({ ...task, dueDate: event.target.value })} /></SpFormField></div><SpFormField id="task-visibility" label="Visibility"><Select value={task.visibility} onChange={(value) => setTask({ ...task, visibility: value })}><option value="ClientVisible">Client visible</option><option value="ProviderPrivate">Provider private</option><option value="SharedTeam">Shared team</option></Select></SpFormField><DialogFooter><Button type="button" variant="outline" onClick={() => setTaskOpen(false)} disabled={createTask.isPending}>Cancel</Button><Button type="button" onClick={saveTask} disabled={!task.title.trim() || createTask.isPending}>{createTask.isPending ? 'Creating…' : 'Create task'}</Button></DialogFooter></DialogContent></Dialog>
