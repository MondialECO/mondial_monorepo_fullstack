...
function MilestoneDetail({ data, milestone, readOnly }: { data: WorkroomDetail; milestone: Milestone; readOnly: boolean }) {
  ...
  const openDispute = async () => {
    ...
    try {
      ...
      setFeedback({ status: 'success', message: 'Dispute opened for support review. Payment is on hold until it resolves.' });
...
function DeliveryDialog({ open, data, milestone, onClose, onSuccess }: { open: boolean; data: WorkroomDetail; milestone: Milestone; onClose: () => void; onSuccess: () => void }) {
  ...
  const linksError = validateHttpUrlList(form.externalLinks);
  const valid = form.title.trim() && form.description.trim() && form.clientInstructions.trim() && (form.fileIds.length > 0 || form.externalLinks.length > 0) && !linksError && form.allDeliverablesIncluded && form.filesReviewed && form.noUnrelatedPrivateInfo && form.readyForReview;
...
function SimpleConfirmation({ kind, pending, onClose, onConfirm }: { kind: Confirmation; pending: boolean; onClose: () => void; onConfirm: () => void }) {
  if (!kind) return null;
  return <Dialog open onOpenChange={(open) => !open && onClose()}><DialogContent><DialogHeader><DialogTitle>{kind === 'activate' ? 'Start this milestone?' : 'Start revision work?'}</DialogTitle><DialogDescription>{kind === 'activate' ? 'The start date is set now and the due date is calculated from the accepted contract.' : 'The revision request will move to In Progress. No scope or pricing decision is made automatically.'}</DialogDescription></DialogHeader><DialogFooter><Button type="button" variant="outline" onClick={onClose} disabled={pending}>Cancel</Button><Button type="button" onClick={onConfirm} disabled={pending}>{pending ? 'Working…' : kind === 'activate' ? 'Start milestone' : 'Start revision'}</Button></DialogFooter></DialogContent></Dialog>;
}
