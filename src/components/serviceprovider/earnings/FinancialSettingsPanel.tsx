  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      <SpCard>
        ...
        <div className="mt-5 rounded-xl border border-[#E5E7EB] bg-[#F9FAFB] p-4"><div className="flex items-start gap-3"><Landmark className="mt-0.5 size-5 text-[#3C61DD]" aria-hidden="true" /><div><p className="text-sm font-semibold text-[#171717]">Minimum payout: {money(data.settings.minimumPayoutAmount, currency)}</p><p className="mt-1 text-xs leading-5 text-[#6B7280]">The minimum and account-hold state are backend-owned. No frontend payout rule is calculated here.</p></div></div></div>
        {data.settings.payoutMethods.length === 0 ? <SpEmptyState className="mt-5 min-h-52 border-0 bg-[#F9FAFB]" icon={CreditCard} title="No payout methods" description="Add a masked descriptor to use with sandbox payout requests." /> : <ul className="mt-5 space-y-3">{data.settings.payoutMethods.map((method) => { const isDefault = method.id === data.settings.defaultPayoutMethodId; return <li key={method.id} className="rounded-xl border border-[#E5E7EB] p-4"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div className="flex min-w-0 items-start gap-3"><span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-[#EEF2FF] text-[#3C61DD]"><CreditCard className="size-5" aria-hidden="true" /></span><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-[#171717]">{method.displayName}</p>{isDefault && <SpStatusBadge tone="info"><Star className="mr-1 size-3" aria-hidden="true" />Default</SpStatusBadge>}<SpStatusBadge tone={method.verified ? 'positive' : 'warning'}>{method.verified ? 'STUB verified' : 'Unverified'}</SpStatusBadge></div><p className="mt-1 text-sm text-[#6B7280]">{words(method.rail)} · {method.maskedDescriptor}</p><p className="mt-1 text-xs text-[#9CA3AF]">Added {formatDate(method.createdAt)}</p></div></div><div className="flex shrink-0 gap-2">{!isDefault && <Button type="button" size="sm" variant="outline" disabled={!method.verified} onClick={() => setMethodAction({ kind: 'default', method })}><BadgeCheck className="size-4" aria-hidden="true" />Make default</Button>}<Button type="button" size="sm" variant="outline" onClick={() => setMethodAction({ kind: 'remove', method })} className="text-[#B42318]"><Trash2 className="size-4" aria-hidden="true" />Remove</Button></div></div></li>; })}</ul>}
      ...
      <SpCard>
        <SpSectionHeader title="Tax & invoice settings" description="These values are stored as the provider’s tax settings and snapshotted onto future invoices." action={<Building2 className="size-5 text-[#3C61DD]" aria-hidden="true" />} />
...
function InvoiceTable({ data }: { data: FinancialSummary }) {
  return <SpCard><SpSectionHeader title="Invoices" description="Immutable invoice records generated from approved payment releases. Corrections reference the original invoice rather than overwriting it." action={<ReceiptText className="size-5 text-[#3C61DD]" aria-hidden="true" />} />{data.invoices.length === 0 ? <SpEmptyState className="mt-5 border-0 bg-[#F9FAFB]" icon={FileText} title="No invoices yet" description="Issued milestone invoices will appear here when the backend creates them." /> : <div className="mt-5 overflow-x-auto"><table className="w-full min-w-[920px] text-left text-sm"><thead><tr className="border-b border-[#E5E7EB] text-xs uppercase tracking-[0.08em] text-[#6B7280]"><th className="px-3 py-3 font-semibold">Invoice</th><th className="px-3 py-3 font-semibold">Project / milestone</th><th className="px-3 py-3 font-semibold">Gross</th><th className="px-3 py-3 font-semibold">Commission</th><th className="px-3 py-3 font-semibold">Net</th><th className="px-3 py-3 font-semibold">Lifecycle</th><th className="px-3 py-3 font-semibold">Status</th></tr></thead><tbody>{data.invoices.map((invoice) => <tr key={invoice.id} className="border-b border-[#E5E7EB] align-top last:border-0"><td className="px-3 py-4"><p className="font-semibold text-[#171717]">{invoice.invoiceNumber}</p>{invoice.correctsInvoiceId && <p className="mt-1 text-xs text-warning">Corrects {shortReference(invoice.correctsInvoiceId)}</p>}</td><td className="px-3 py-4 text-[#374151]"><p>Project {shortReference(invoice.engagementId)}</p><p className="mt-1 text-xs text-[#6B7280]">Milestone {shortReference(invoice.milestoneId)}</p></td><td className="px-3 py-4 text-[#374151]">{money(invoice.grossAmount, invoice.currency)}</td><td className="px-3 py-4 text-[#374151]">{money(invoice.commissionAmount, invoice.currency)}</td><td className="px-3 py-4 font-semibold text-[#171717]">{money(invoice.netAmount, invoice.currency)}</td><td className="px-3 py-4 text-xs text-[#6B7280]"><p>Approved {formatDate(invoice.approvalDate)}</p><p className="mt-1">Released {formatDate(invoice.releaseDate)}</p></td><td className="px-3 py-4"><SpStatusBadge tone={transactionTone(invoice.status)}>{words(invoice.status)}</SpStatusBadge></td></tr>)}</tbody></table></div>}<p className="mt-5 text-xs leading-5 text-[#6B7280]">The API does not return invoice PDFs, downloadable templates, editable numbering, or a document-generation endpoint, so none are presented here.</p></SpCard>;
}

function ReviewRow({ label, value }: { label: string; value: string }) { return <div className="flex items-start justify-between gap-4"><dt className="text-sm text-[#6B7280]">{label}</dt><dd className="text-right text-sm font-semibold text-[#171717]">{value}</dd></div>; }
