...
export function FinancialSettingsPanel({ data, currency }: { data: FinancialSummary; currency: string }) {
  ...
  const saveMethod = async () => {
    ...
    try {
      ...
      setFeedback({ status: 'success', message: 'Masked payout method saved and marked verified.' });
  ...
  const confirmMethodAction = async () => {
    ...
    try {
      if (methodAction.kind === 'default') {
        ...
        setFeedback({ status: 'success', message: `${methodAction.method.displayName} is now the default payout method.` });
      } else {
