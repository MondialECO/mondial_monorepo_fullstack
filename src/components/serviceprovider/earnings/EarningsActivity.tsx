  ...
  return <div className="space-y-6">
    ...
    <SpCard>
      ...
      <SpFilterBar className="mt-5 border-0 bg-[#F9FAFB] p-3">
        <label className="relative min-w-0 flex-1"><span className="sr-only">Search transactions</span><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-[#6B7280]" aria-hidden="true" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search type or reference" className="bg-white pl-9" /></label>
        <label><span className="sr-only">Transaction status</span><select value={status} onChange={(event) => setStatus(event.target.value)} className="h-10 w-full rounded-xl border border-[#D1D5DB] bg-white px-3 text-sm text-[#374151] outline-none focus-visible:ring-2 focus-visible:ring-[#3C61DD]"><option value="all">All statuses</option>{statuses.map((item) => <option key={item} value={item}>{words(item)}</option>)}</select></label>
...
function StatementPanel({ currency }: { currency: string }) {
  ...
  return <SpCard>
    <SpSectionHeader title="Financial statement" description="Generate a structured server response for a date range. No PDF or downloadable statement is available in the current API." action={<FileBarChart2 className="size-5 text-[#3C61DD]" aria-hidden="true" />} />
