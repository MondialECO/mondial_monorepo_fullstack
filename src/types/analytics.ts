...
export interface ProfileFunnel { briefsShown: AnalyticsMetric; proposalsSent: AnalyticsMetric; hired: AnalyticsMetric; proposalRate: AnalyticsMetric; hireRate: AnalyticsMetric }
export interface TopService { serviceId: string; title: string; clicks: number; impressions: number }

export interface ProfileAnalytics { funnel: ProfileFunnel; topServices: TopService[]; trustScore: AnalyticsMetric; trustSignals: TrustSignalAnalytics[]; disputePenalty: AnalyticsMetric; profileCompleteness: AnalyticsMetric; verificationStatus: string; tierLevel: number; tierMeaning: string; skillsTestsTaken: AnalyticsMetric; skillsTestsPassed: AnalyticsMetric; latestSkillsTestScore: AnalyticsMetric; portfolioItems: AnalyticsMetric; publishedServices: AnalyticsMetric; profileViews: AnalyticsMetric; searchAppearances: AnalyticsMetric; portfolioViews: AnalyticsMetric; profileSaves: AnalyticsMetric; contactRate: AnalyticsMetric; portfolioEngagement: AnalyticsMetric }
export interface AnalyticsBreakdown { key: string; label: string; gross: number; commission: number; net: number; count: number }
/**
 * Released revenue split by how the client arrived, weighted by NET REVENUE rather than
 * deal count — see ClientSourceAnalyticsResponse on the server for why. unattributedNet is
 * revenue that could not be traced to a proposal; it is excluded from the two percentages
 * rather than folded into either channel.
 */
