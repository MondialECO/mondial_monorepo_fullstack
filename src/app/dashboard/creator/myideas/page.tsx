...
export default function MyIdeasPage() {
  ...
  if (!hasIdea) {
    return (
      <div className="mx-auto w-full max-w-[1140px] py-10 px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mt-10">
          ...
          {/* Card 2: Help me discover an idea */}
          <Card className="flex flex-col justify-between rounded-2xl border border-border bg-card p-6 sm:p-8 hover:border-warning/50 hover:shadow-md transition-all duration-300">
            <div className="space-y-6">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-warning/10 text-warning">
                <Compass className="h-6 w-6" />
              </div>
              <div className="space-y-2">
                <h3 className="text-lg font-bold text-foreground sm:text-xl">Help me discover an idea</h3>
                <p className="text-xs sm:text-sm leading-relaxed text-muted-foreground">
            ...
            <Button
              onClick={handleDiscoveryPath}
              variant="outline"
              className="group mt-6 w-full rounded-xl py-5 text-xs font-bold sm:text-sm border-warning text-warning hover:bg-warning/5"
            >
  ...
  if (!isPhase2Complete) {
    const currentStepLabel = (() => {
      ...
      if (step === 2) return "Idea Discovery Inputs";
      if (step === 3) return "AI Synthesis Processing";
      if (step === 4) return "Reviewing Idea Candidates";
      if (step === 5) return "Confirming Selected Concept";
