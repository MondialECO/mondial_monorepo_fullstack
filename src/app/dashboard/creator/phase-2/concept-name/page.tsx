...
import { Sparkles, ArrowRight, RefreshCw, Check, Lightbulb, Type, Globe, Lock, ArrowLeft } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useCreatorProgress } from "@/providers/CreatorProgressProvider";

const AI_SUGGESTIONS = [
  { name: "BrandKit AI", tagline: "Instant brand identity for local businesses.", score: 94 },
  { name: "LogoMint", tagline: "Mint your brand in minutes.", score: 87 },
  { name: "Identiq", tagline: "Your identity. Your brand. Instantly.", score: 82 },
  { name: "Kitsuki", tagline: "Beautiful brand kits, beautifully simple.", score: 79 },
  { name: "Brandora", tagline: "AI-powered branding for the next billion.", score: 76 },
];
...
export default function ConceptNamePage() {
  ...
  useEffect(() => {
    ...
    if (project) {
      if (project.name) {
        ...
        const sugIndex = AI_SUGGESTIONS.findIndex((s) => s.name === project.name);
        if (sugIndex !== -1) {
  ...
  const activeName = useCustom
    ...
    ? AI_SUGGESTIONS[selectedSuggestion].name
    : "";
  const activeTagline = useCustom
    ? customTagline
    : selectedSuggestion !== null
    ? AI_SUGGESTIONS[selectedSuggestion].tagline
    : "";

  const canProceed = activeName.trim().length >= 2 && activeTagline.trim().length >= 5;
          ...
          <div className="flex-1 space-y-8">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button className="flex items-center gap-1.5 text-xs text-primary font-bold hover:opacity-85">
