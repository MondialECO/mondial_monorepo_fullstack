...
const QUESTIONS = [
  "", // Q0 Placeholder
  "Welcome. Tell me your idea — in your own words, however rough it is. What are you trying to build?",
  "Got it. Now tell me — who specifically is struggling with this the most? Are we talking about designers, developers, writers, or someone else?",
  "Understood. What is the current way they handle this issue, and why does it fail or remain frustrating?",
  "Great. How will your solution automatically solve this without making it feel awkward? What's the secret sauce?",
  "Fascinating. What is the market gap or your unique edge that ensures competitors can't easily copy this?",
  "Excellent! I have successfully clarified your business idea. Your business canvas is ready and your Idea Score is 100/100. Let's review the first draft!",
];
...
export default function AIClarifierPage() {
  ...
  useEffect(() => {
    ...
    if (savedMessages && savedMessages.length > 0) {
      ...
      const initialMsgs: Message[] = [{ id: "1", sender: "ai", text: QUESTIONS[1] }];
  ...
  const handleSend = () => {
    ...
    // Update Project Fields dynamically in state
    ...
    setIdeaScore(Math.min((nextStep - 1) * 20, 100));

    // Save user message and updated project state immediately
    setState((prev) => ({
      ...prev,
      project: {
        ...prev.project,
    ...
    setTimeout(() => {
      setIsTyping(false);
      const aiMsgText = QUESTIONS[nextStep];
      const aiMsgId = (Date.now() + 1).toString();
      const finalMessages = [...updatedMessages, { id: aiMsgId, sender: "ai" as const, text: aiMsgText }];
            ...
            {currentStep <= 5 ? (
              ...
              <div className="w-full flex flex-col items-center gap-3 py-4 text-center">
                ...
                <Button
                  ...
                  className="font-semibold py-2.5 px-6 rounded-xl flex items-center gap-2 text-xs shadow-sm"
                >
