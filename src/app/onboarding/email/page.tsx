...
export default function OnboardingEmailPage() {
  ...
  async function devConfirm() {
    ...
    setBusy(true);
    try {
      await api.post("/onboarding/email/dev-confirm");
      await refresh();
      router.push("/onboarding");
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message;
  ...
  return (
    <div className="mx-auto max-w-[560px] px-4 sm:px-6 py-10 sm:py-14 space-y-6">
      ...
      {/* Development shortcut — mirrors the phone/identity page. */}
      <div className="rounded-2xl border border-dashed border-border bg-card p-5 space-y-3 mt-6">
        <div className="flex items-start gap-3 text-xs text-amber-900 dark:text-amber-200">
          <AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" />
          <span>
            Development shortcut: skip the email exchange and mark the email verified. Available only while the backend runs in Development.
          </span>
        </div>
