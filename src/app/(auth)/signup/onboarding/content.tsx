...
import { getAuthErrorMessage, registerApi, validateOnboardingToken } from '@/lib/api-auth';
...
import { parseBackendUserRole } from '@/lib/roles';
...
export default function SignupOnboardingContent() {
  ...
  const [postSignupRole, setPostSignupRole] = useState<string | null>(null);
  const [tokenError, setTokenError] = useState<string | null>(null);
  const [isValidatingToken, setIsValidatingToken] = useState(false);
  const { setAuthSession } = useAuth();
  const onboardingToken = searchParams.get('token');
  const isPostSignupMode = Boolean(onboardingToken);
  ...
  useEffect(() => {
    ...
    const validateToken = async () => {
      ...
      try {
        ...
        setPostSignupEmail(result.email);
        setPostSignupRole(result.role);
      } catch (error) {
      ...
      if (token && apiUser) {
        ...
        if (role) {
          setAuthSession(token, {
            ...
            name: String(apiUser.name ?? apiUser.Name ?? data.fullName),
            role,
