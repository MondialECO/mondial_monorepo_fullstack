...
namespace WebApp.Controllers
{
    ...
    public class OnboardingController : ControllerBase
    {
        ...
        public class CreatorSetupRequest
        {
        ...
        [HttpPost("creator-setup")]
        [Consumes("multipart/form-data")]
        public async Task<IActionResult> CreatorSetup([FromForm] CreatorSetupRequest req)
        {
            var user = await CurrentUserAsync();
            if (user == null) return Fail("User not found", 404);
            ...
            user.linkedin_url = req.LinkedinUrl ?? user.linkedin_url;
            
            if (!string.IsNullOrEmpty(req.Skills))
            {
                user.CreatorProfile.Categories = req.Skills.Split(',', StringSplitOptions.RemoveEmptyEntries).Select(s => s.Trim()).ToList();
            }

            if (!string.IsNullOrEmpty(req.PortfolioUrl))
            {
                if (user.CreatorProfile.SocialProfiles == null) user.CreatorProfile.SocialProfiles = new List<string>();
                if (!user.CreatorProfile.SocialProfiles.Contains(req.PortfolioUrl)) {
                    user.CreatorProfile.SocialProfiles.Add(req.PortfolioUrl);
                }
            }
            ...
            if (req.Avatar != null && req.Avatar.Length > 0)
            {
                try
                {
                    user.ImagePath = await _fileService.SaveFileAsync(req.Avatar, "avatars");
                }
                catch (ArgumentException ex)
                {
                    return Fail(ex.Message);
            ...
            // Bypass universal KYC for Creators to simplify onboarding for now
            user.Onboarding.Phase = 1;
            user.Onboarding.CompletedAt = DateTime.UtcNow;
            user.KycStatus = "VERIFIED";
            user.Kyc.Status = VerificationStatus.Verified;
            ...
            await _userManager.UpdateAsync(user);
            return Ok("Creator profile setup complete");
        }
    }
}
