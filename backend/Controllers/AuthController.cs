...
namespace WebApp.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        private IActionResult Fail(string message, object data)
        {
            return BadRequest(new
            {
                success = false,
        ...
        public async Task<IActionResult> Register(RegisterModel model)
        {
            ...
            if (!result.Succeeded)
            {
                ...
                return Fail(errors.FirstOrDefault() ?? "User registration failed", errors);
            }
            ...
            var addRoleResult = await _userManager.AddToRoleAsync(user, canonicalRole);
            if (!addRoleResult.Succeeded)
            {
                var errors = addRoleResult.Errors.Select(error => error.Description).ToArray();
                return Fail(errors.FirstOrDefault() ?? "Could not assign account role", errors);
            }
            ...
            {
                Token = JwtTokenHelper.GenerateRefreshToken(),
                ExpiresAt = DateTime.UtcNow.AddDays(7),
                CreatedByIp = HttpContext.Connection.RemoteIpAddress?.ToString() ?? ""
            };
            user.LastLogin = DateTime.UtcNow;
