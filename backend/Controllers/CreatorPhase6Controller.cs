    ...
    public class CreatorPhase6Controller : ControllerBase
    {
        ...
        private async Task<CreatorReadinessResponse> BuildReadinessAsync(string userId, CreatorJourney journey)
        {
            ...
            var requirements = new List<CreatorReadinessRequirement>
            {
                ...
                new() { Key = "commercial_preparation", Label = "Prepare your commercial offer", Route = "/dashboard/creator/phase-4", Complete = computed.Phase4.Status == "completed", Required = true, Blocking = true, Status = computed.Phase4.Status == "completed" ? "COMPLETE" : "PENDING" },
                    ...
                    else
                    {
                        ...
                        if (existingConcept == null)
                        {
                            var concept = new Phase3Concept
                            {
                                ...
                                BusinessModel = journey.Phase4Data?.PricingStrategy?.PrimaryRevenueModel.ToString() ?? journey.Project?.Category ?? "SaaS_Subscription",
